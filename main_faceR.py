...
# 10) Create objecter configuration file. done V 12-05-2022 (temp code in main_face_05-archive)
# 11) Create user interace. ongoing (re-use Custom Tkinter master)
#       11a) Create message broker for messaging between different users
# 12) Make a routine for copying unknown faced to images (supervised so need to use UI)
# 13) Make executable
...
def faceR(cam):
    ...
    Uface = store_unknown('Unknown_faces', 'rename me please') # Store Unknown faces for proof or addition to library
    ...
    cap = cv2.VideoCapture(cam, cv2.CAP_DSHOW) # Center Cam
    ...
    while True:
        ...
        windowName = "Face recognition camera: " + str(cam)
